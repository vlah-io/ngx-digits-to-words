...
const directoryPath = path.join(__dirname, 'projects/ngx-digits-to-words/src/lib');
const results = [];
...
try {
  ...
  const fp = fs.createWriteStream(path.join(__dirname, 'projects/ngx-digits-to-words/src/public-api.ts'), {
    flags: 'w'
  });
  results.forEach((str) => {
